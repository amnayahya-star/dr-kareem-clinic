...
import { Stethoscope, UserCheck, Activity, ArrowLeft, CheckCircle2 } from "lucide-react";
...
export default function HomePage() {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between p-4 sm:p-8">
      ...
      <header className="max-w-5xl mx-auto w-full flex items-center justify-between py-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center shadow-sm">
            <Activity className="w-5 h-5 text-clinic-400" />
          </div>
          <div>
        ...
        <Link
          href="/login"
          className="text-xs font-bold text-slate-700 hover:text-slate-900 bg-white border border-slate-200 px-4 py-2 rounded-xl shadow-xs hover:border-slate-300 transition-all"
        >
          تسجيل الدخول
        </Link>
      </header>
      ...
      <main className="max-w-3xl mx-auto w-full my-auto text-center py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-right">
          ...
          >
            ...
            <p className="text-xs text-slate-500 mt-2 leading-relaxed">
              إضافة طفل جديد، البحث الفوري لفتح زيارة وتسجيل القياسات، ومتابعة الانتظار وطباعة الوصفات.
            </p>
      ...
      {/* Footer */}
      <footer className="max-w-5xl mx-auto w-full text-center text-xs text-slate-400 py-3 border-t border-slate-200/60">
        عيادة الدكتور عبد الكريم عليوي - بورد طب الأطفال وحديثي الولادة
      </footer>
    </div>
  );
}
