...
function mapSupabaseRowToPatientFile(row: any): PatientFile {
  ...
  const visits: VisitRecord[] = (row.visits || []).map((v: any): VisitRecord => {
    ...
    return {
      ...
      status: v.status || (v.is_completed ? "completed" : "waiting"),
      ...
      symptoms: d.symptoms,
      presentIllnessHistory: d.present_illness_history,
      ...
      doctorId: d.doctor_id || v.doctor_id,
      secretaryId: v.secretary_id,
